...
import NoteTypeSelector from '../CreateNote/NoteTypeSelector';
import TextNoteEditor from '../CreateNote/NoteEditor';
import TodoNoteEditor from '../CreateNote/TodoNoteEditor';
import CodeNoteEditor from '../CreateNote/CodeNoteEditor';
import DrawingCanvas from '../CreateNote/DrawingCanvas';
import AudioRecorder from '../CreateNote/AudioRecorder';
import NoteTemplates from '../NoteTemplates/NoteTemplates';
import { noteService } from '../../../services/noteService';

const EditNote = ({ note, darkMode, onClose, onSave, folders = [] }) => {
  const [selectedType, setSelectedType] = useState(note?.type || 'text');
  ...
  useEffect(() => {
    if (note) {
      setNoteData({
        ...
        color: note.color || (darkMode ? 'from-blue-900/50 to-blue-800/50' : 'from-blue-100 to-blue-200'),
  ...
  const handleSave = async () => {
    ...
    if (!note?._id) {
      setError('Cannot update: Invalid note ID');
      return;
    }
    ...
    try {
      ...
      console.log(' Updating note:', finalNote);
      
      const result = await noteService.updateNote(note._id, finalNote);
      ...
      setError(error.message || 'Failed to update note. Please check your connection and try again.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!note) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
        <div className={`relative rounded-2xl shadow-2xl max-w-md w-full p-6 ${
          ...
          <div className="text-center">
            ...
            <p className={`mb-4 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              The note you're trying to edit could not be found.
            </p>
            <button
              onClick={onClose}
            ...
            <div className="flex gap-3">
              ...
              >
                {isLoading ? (
                  ...
                  currentStep === 'details' ? 'Update Note' : 'Continue'
                )}
              </button>
            </div>
          </div>
